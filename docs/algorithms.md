...
> time and space complexity, implementation decisions, and answers to common
...
`weight` is a 0–10 score where **lower means easier to exploit**. Authors
pre-compute this from CVSS / exploitability data and put it directly on the
edge; the loader uses the value as-is (no inversion is applied).

- A trivially exploitable edge (default creds, public RCE) gets `weight ≈ 1.0`.
- A merely possible edge (requires auth, internal-only) gets `weight ≈ 7.0`–`9.0`.
- The default when `weight` is omitted is `5.0`.

Dijkstra minimizes the sum of `weight` along a path, so it naturally finds the
**easiest** route for an attacker. The full field reference (including how
`weight` differs from the displayed `risk`) lives in
[docs/CLUSTER_GRAPH_SCHEMA.md](docs/CLUSTER_GRAPH_SCHEMA.md).
...
### Edge weight semantics — critical design detail

Edge `weight` is **exploitability cost** on a 0–10 scale where **lower means
easier to exploit**. Authors set this value directly in the input file — the
loader does **not** derive it from `risk` (there is no `10 - risk` inversion).
Dijkstra minimizes the sum of `weight` along a path, so it naturally finds
the attacker's preferred route — the one made up of the easiest hops.

Example:
```
web-server     --[uses,        weight=2.5]--> backend-sa
backend-sa     --[bound-to,    weight=0.5]--> admin-role
admin-role     --[can-read,    weight=1.0]--> db-credentials
db-credentials --[unlocks,     weight=0.5]--> billing-db

total_cost = 2.5 + 0.5 + 1.0 + 0.5 = 4.5
```

A `total_cost` of 4.5 out of a possible 40 (4 hops × 10) means this path is
highly exploitable. The response also surfaces a per-edge `risk` value
(displayed alongside each hop) which is independent of `weight`; see
[docs/CLUSTER_GRAPH_SCHEMA.md](docs/CLUSTER_GRAPH_SCHEMA.md#edge-weight-semantics-read-this-carefully--c-3)
...
| Neo4j | Yes | ❌ Requires running a separate database process. Overkill at current scale. Would use for production (> 10k nodes). |
...
Graph algorithms give us **explainability** — we can show
security teams the exact path, with the exact edge weights,
...
At the current scale, SQLite is the right default:
